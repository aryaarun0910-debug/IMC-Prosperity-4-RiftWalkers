"""R4 gameday orchestrator (created 2026-04-26 post-R3-close).

One command from the PIPELINE/ directory after dropping new files into
data/r4/drop/:

    py -3.12 tools/r4_kickoff.py

Pipeline:
  1. sort_drops --round 4            (fans drop/ into prices/trades/observations/server_logs/wiki/intel)
  2. first_log_intel on any new server logs
  3. r3_preflight                    (10-check fortress gate — generic, reused)
...
R4_DIR = os.path.join(PIPELINE_DIR, "data", "r4")
...
def list_logs():
    sl = os.path.join(R4_DIR, "server_logs")
...
def main():
    ...
    ap.add_argument("--skip-preflight", action="store_true",
                    help="Skip the preflight fortress gate")
    args = ap.parse_args()

    print(f"\nR4 KICKOFF — {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"PIPELINE_DIR={PIPELINE_DIR}")
    print(f"R4_DIR={R4_DIR}")

    if not os.path.isdir(R4_DIR):
        print(f"ERROR: R4 folder missing at {R4_DIR}. Create it first.")
        sys.exit(1)

    results = {}

    logs_before = set(list_logs())
    results["sort_drops"] = run("py -3.12 tools/sort_drops.py --round 4",
                                "Sort drop/ into r4/ subfolders")
    ...
    if not args.skip_preflight:
        results["preflight"] = run("py -3.12 tools/r3_preflight.py",
                                   "Preflight fortress gate")
