...
- Backtested against the available day: projected ~130,000 XIRECs.
- Delivered on the unseen scoring day: **19,664 XIRECs.**

On the development day the underlying drifted down monotonically and the thesis worked. On the scoring day the underlying barely moved on net but spiked intraday, driving the short option book through large mark-to-market drawdowns (one strike: peak +7,633 XIRECs, trough −20,991 XIRECs, close −5,781 XIRECs). Simultaneously, the supposedly-reliable market-making products returned roughly half their backtested PnL, pure single-day variance on identical code.
...
|-----------|-------------------|----------------------------|------------------|
| Hardcoded direction | 174,603 XIRECs | +44,486 XIRECs | **−33,050 XIRECs** |
| Cross-day validated | 34,505 XIRECs | **+615,749 XIRECs** | **+143,139 XIRECs** |
...
Even with this discipline, the Round 5 algorithm delivered only +6,849 XIRECs on the live day. The validated mean-reversion approach has its own genuine failure mode (it gives back PnL on strongly trending days), and three development days is a thin statistical jury. The lesson is not that cross-day validation guarantees success; it is that single-day calibration guarantees fragility, and the gap between those two is the difference between a controlled loss and a blow-up.
